"""ONE authorized sampling-repair child of the capped complex-terrain run. Never a regression test."""
...
out=ROOT/'evidence/hybrid-sampling-repair';out.mkdir(exist_ok=True)
...
s=default_store();pid='1be76cabfa584293be43c418653d9a40'
p,_,cfg=validated_sampling_repair(s,parent_id=pid)
assert p['inherited_calls']==5 and p['inherited_images']==1 and p['inherited_local']==0
assert p['corrected_binding']['crops']['square']['source_pixels_per_unit']==[64,64]
assert p['corrected_binding']['crops']['water']['source_pixels_per_unit']==[64,64]
...
assert account['effective_microusd']==16516937 and account['settled_calls']==4 and len(unknown)==2
assert not s.g.policy['approved'] and s.g.policy['total_usd']=='20.00'
private=ROOT/'data/hybrid-sampling-repair-before.json'
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=180) as c:
    response=c.post('/api/hybrid-runs/'+pid+'/continue-sampling-repair',json=dict(budget_microusd=budget,max_seconds=7200,confirm_paid=True,approval_text=approval),headers={'Idempotency-Key':'sampling-repair-calls2-images0-once-v1'})
    response.raise_for_status();r=response.json();immutable(out/'new-run.json',canonical(r))
    assert (r['call_count'],r['image_count'],r['local_count'])==(5,1,0)
    assert r['config']['max_calls']==7 and r['config']['max_local_corrections']==1
    assert r['config']['followup_call_limit']==2 and r['config']['no_new_images'] is True
    assert r['config']['continuation']['kind']=='sampling_repair'
    scope=dict(run_id=r['id'],config_sha256=r['config_sha256'],approval_text=approval,amendment=r['config']['authorization_amendment'],
        parent_id=pid,project_cap_microusd=20000000,budget_microusd=budget,max_calls=7,max_images=1,new_images=0,
        followup_calls=2,settled_calls=4,expires=time.time()+7200,
