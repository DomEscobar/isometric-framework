"""Create one fresh, EUR50-capped terrain run: one-schema transport + explicit token policy."""
...
OUT=ROOT/'evidence/code-layout-flat-live-v12';OUT.mkdir(parents=True,exist_ok=True)
if (OUT/'new-run.json').exists():raise SystemExit('already started')
parent_id='6de8935071bc4a01bb41b88e00c7603e'
...
# Explicit bounded token-policy change (truncation remedy): planner 65536/medium.
body['token_policy']=copy.deepcopy(cfg['token_policy']);body['token_policy']['planner']=dict(max_tokens=65536,reasoning_effort='medium')
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=120) as c:
    ...
    resp=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'eur50-flat-terrain-recovery-v12-20260923'})
    ...
    scope=dict(run_id=run['id'],config_sha256=run['config_sha256'],approval_text='Same explicit EUR50 cap and quality recovery authorization; fresh run after single-schema transport fix and explicit planner token policy 65536/medium.',project_cap_microusd=57_315_000,budget_microusd=budget,max_calls=36,max_images=15,expires=time.time()+7200,liabilities=[dict(call_id=u['id'],request_sha256=digest(u['request'].encode()),amount=u['amount']) for u in unknown])
