...
class MirroredChainTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(); self.addCleanup(self.temp.cleanup)
        self.root = Path(self.temp.name); self.art = self.root / "host/art"; self.art.mkdir(parents=True)
        frames = self.art / "source-frames"; frames.mkdir()
        ...
        self.write_path(recipe_path, recipe)
        extractor.export(recipe_path, self.art / "extract")
        packer = self.script("pack-sprites.py", "packer")
        packed_ne = packer.pack(self.art / "extract/sprite-pack.json", self.art / "packed")
        mirrorer = self.script("mirror-frames.py", "mirrorer")
        mirrorer.mirror(self.art / "extract/sprite-pack.json", self.art / "extract-nw", "nw")
        packed_nw = packer.pack(self.art / "extract-nw/sprite-pack.json", self.art / "packed-nw")
        self.merge_runtime(packed_ne, packed_nw)
        self.write("host/art/video-job.json", {"localJobId": "video-fixture", "provider": "fixture", "model": "ffv1-fixture", "selectedImageSha256": self.sha(self.facing), "videoSha256": self.sha(self.art / "review/source/motion.mkv")})
        origin = self.origin()
        self.write("host/art/ledger.json", {"version": 1, "images": {"hero": {"origin": origin, "transforms": [{"path": "host/art/sheet.png", "sha256": self.sha(self.art / "sheet.png")}]}}, "clips": {
            "hero.walk.ne": {"mode": "image-to-video-extract-pack", "selectedImage": origin, "videoJob": self.evidence("host/art/video-job.json"), "recipe": self.evidence("host/art/review/extraction.json")},
            "hero.walk.nw": {"mode": "mirrored-frames", "sourceClip": "hero.walk.ne", "mirror": {"sourceDirection": "ne", "direction": "nw"}},
        }})
        self.plan = {"version": 4, "inputRoots": ["host"], "assetPolicy": {"version": 1, "sources": {"world": "generated", "character": "generated", "environment": "generated"}, "characterAnimation": {"animated": "image-to-video-extract-pack", "staticIdle": "generated-facing"}, "coverageLedger": "host/art/ledger.json", "runtime": {"manifest": "host/art/runtime.json", "binding": "host/art/used.json"}}}

    def merge_runtime(self, packed_ne, packed_nw):
        with Image.open(self.art / "packed/sheet.png") as north, Image.open(self.art / "packed-nw/sheet.png") as west:
            combined = Image.new("RGBA", (max(north.width, west.width), north.height + west.height))
            combined.paste(north, (0, 0)); combined.paste(west, (0, north.height)); dy = north.height
            combined.save(self.art / "sheet.png")
        textures = dict(packed_ne["assets"]["textures"])
        for texture_id, texture in packed_nw["assets"]["textures"].items():
            frame = dict(texture["frame"]); frame["y"] = frame["y"] + dy
            textures[texture_id] = {**texture, "image": "hero", "frame": frame}
        animations = {**packed_ne["assets"]["animations"], **packed_nw["assets"]["animations"]}
        self.write("host/art/runtime.json", {"assets": {
            "images": {"hero": {"url": "sheet.png", "sampling": "nearest"}},
            "textures": textures, "animations": animations}})
        self.write("host/art/used.json", {"version": 1, "used": {category: {"images": (["hero"] if category == "character" else []), "textures": (list(textures) if category == "character" else []), "animations": (list(animations) if category == "character" else [])} for category in ("world", "character", "environment", "ui", "debug")}})
    ...
    def test_mirrored_facing_replays_from_source_recipe(self):
        self.assertEqual(gate.verify(self.plan, self.root)["characterClips"], 2)

    def test_rejects_front_back_pair_and_missing_source(self):
        ledger = self.ledger()
        ledger["clips"]["hero.walk.nw"]["mirror"] = {"sourceDirection": "ne", "direction": "s"}
        self.save_ledger(ledger)
        with self.assertRaisesRegex(ValueError, "horizontal facing"):
            gate.verify(self.plan, self.root)
        ledger = self.ledger()
        ledger["clips"]["hero.walk.nw"]["mirror"] = {"sourceDirection": "ne", "direction": "nw"}
        ledger["clips"]["hero.walk.nw"]["sourceClip"] = "hero.walk.se"
        self.save_ledger(ledger)
        with self.assertRaisesRegex(ValueError, "extracted character clip"):
            gate.verify(self.plan, self.root)

    def test_rejects_changed_mirrored_pixels(self):
        sheet = Image.open(self.art / "sheet.png"); sheet.putpixel((3, sheet.height - 3), (1, 2, 3, 255)); sheet.save(self.art / "sheet.png")
        ledger = self.ledger(); ledger["images"]["hero"]["transforms"][-1]["sha256"] = self.sha(self.art / "sheet.png"); self.save_ledger(ledger)
        with self.assertRaisesRegex(ValueError, "frame pixels"):
            gate.verify(self.plan, self.root)
