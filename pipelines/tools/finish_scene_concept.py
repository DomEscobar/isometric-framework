...
out=ROOT/'evidence/hybrid-scene-concept'
r=json.loads((out/'new-run.json').read_bytes())
assert r['config']['continuation'].get('kind')=='directive_repair' and r['config']['followup_call_limit']==5
assert (r['call_count'],r['image_count'])==(11,3) and r['config']['max_images']==5 and r['config']['max_calls']==16
assert r['config']['authorization_amendment']['flow']=='scene_concept'
s=default_store()
...
assert account['effective_microusd']+2760936<=20000000 and len(unknown)==2
budget=r['config']['budget_microusd']
assert budget==2900000 and budget+account['effective_microusd']<=20000000
approval=(ROOT/'HYBRID_COMPLEX_TERRAIN_AUTHORIZATION.md').read_text()
assert len(approval)<=4000
scope=dict(run_id=r['id'],config_sha256=r['config_sha256'],approval_text=approval,amendment=None,
    parent_id='2222f03c528e47a099e17606ef6e0307',project_cap_microusd=20000000,budget_microusd=budget,
    max_calls=16,max_images=5,new_images=2,
