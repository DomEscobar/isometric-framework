"""Bind a fresh run to the current checker/compiler and exact liability set."""
...
OUT=ROOT/'evidence/code-layout-flat-live-v9';OUT.mkdir(parents=True,exist_ok=True)
if (OUT/'new-run.json').exists():raise SystemExit('already started')
old=json.loads((ROOT/'evidence/code-layout-flat-live-v8/new-run.json').read_bytes());store=default_store()
...
assert {u['id'] for u in unknown}=={'ff21a6ffd8500f6d9139eb1b71677078271381cd0f26f6291750a30d6deb696e','79320908cff658062cc43764eb539e5d706a5f747029a98e62c212ef5011357a'}
parent=store.get(old['id']);assert parent['phase']=='needs_attention' and parent['image_count']==0 and parent['call_count']==6
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=120) as c:
 ...
 resp=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'eur50-flat-terrain-recovery-v9-20260923'})
 ...
 scope=dict(run_id=run['id'],config_sha256=run['config_sha256'],approval_text='Same explicit EUR50 cap and quality recovery authorization; fresh runtime-bound run after checker fix.',project_cap_microusd=57_315_000,budget_microusd=budget,max_calls=36,max_images=15,expires=time.time()+7200,liabilities=[dict(call_id=u['id'],request_sha256=digest(u['request'].encode()),amount=u['amount']) for u in unknown])
 ...
 print(json.dumps({'run_id':run['id'],'phase':rb.json()['phase'],'calls_already_used':6,'max_calls':36,'project_cap_microusd':57315000,'effective_before':before['effective_microusd'],'run_budget':budget,'unknown_holds':len(unknown),'authorization_id':aid,'runtime':runtime_version()}))
