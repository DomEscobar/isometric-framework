...
out=ROOT/'evidence/hybrid-board-variants2';out.mkdir(exist_ok=True)
...
s=default_store();pid='5257bcc281934e90bbe0e52d543d48e2'
p,_,cfg=validated_directive(s,parent_id=pid)
assert p['inherited_calls']==9 and p['inherited_images']==2 and p['defects']
...
assert account['effective_microusd']==14805453 and account['settled_calls']==9 and len(unknown)==2
assert not s.g.policy['approved'] and s.g.policy['total_usd']=='20.00'
private=ROOT/'data/hybrid-board-variants2-before.json'
...
directive=('Owner explicit choice 2026-09-22, second attempt with CORRECTED rules: ALL seven slots stay strictly FLAT face-on orthographic material interiors. plateau = plain flat grassy turf like land but slightly drier tone - absolutely NO cliff, NO tier, NO elevation edge, NO overhang, NO cutaway. water = smooth empty flat water surface only - absolutely NO lotus, NO lily pads, NO plants, NO objects. Keep the richer fine variation of the improved slots: lively grass tufts, warm fine sand, small varied cobbles, subtle flat cut stone for stairs (NO treads/risers), crisp ashlar wall masonry. Finer joints and feature scale; the renderer builds all geometry.')
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=180) as c:
    response=c.post('/api/hybrid-runs/'+pid+'/continue-directive',json=dict(budget_microusd=budget,max_seconds=7200,confirm_paid=True,approval_text=approval,owner_style_directive=directive),headers={'Idempotency-Key':'board-variants2-calls4-images1-once-v1'})
    response.raise_for_status();r=response.json();immutable(out/'new-run.json',canonical(r))
    assert (r['call_count'],r['image_count'])==(9,2) and r['config']['max_images']==3 and r['config']['max_calls']==13
    ...
    scope=dict(run_id=r['id'],config_sha256=r['config_sha256'],approval_text=approval,owner_style_directive=directive,
        ...
        max_calls=13,max_images=3,new_images=1,followup_calls=4,settled_calls=9,expires=time.time()+7200,
