...
E=ROOT/'evidence/generated-character-check';E.mkdir(exist_ok=True)
...
if not (E/'binding.json').exists():
    ...
    for group in ['evidence/village-material-loop/v4','evidence/character-terrain-check']:
        save('preserved-'+Path(group).name+'.json',{str(x.relative_to(ROOT)):digest(x.read_bytes()) for x in (ROOT/group).rglob('*') if x.is_file()})
