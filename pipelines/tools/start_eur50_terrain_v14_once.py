"""Create one fresh, EUR50-capped terrain run: stair treads/risers render fix."""
...
OUT=ROOT/'evidence/code-layout-flat-live-v14';OUT.mkdir(parents=True,exist_ok=True)
if (OUT/'new-run.json').exists():raise SystemExit('already started')
parent_id='3515cdd1f48c4374baeb412642a097bc'
...
parent=store.get(parent_id);assert parent['phase']=='needs_attention' and parent['image_count']==0 and parent['call_count']==9
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=120) as c:
    ...
    resp=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'eur50-flat-terrain-recovery-v14-20260923'})
    ...
    scope=dict(run_id=run['id'],config_sha256=run['config_sha256'],approval_text='Same explicit EUR50 cap and quality recovery authorization; fresh run after stair tread/riser/flank rendering fix for the sole remaining review criterion.',project_cap_microusd=57_315_000,budget_microusd=budget,max_calls=36,max_images=15,expires=time.time()+7200,liabilities=[dict(call_id=u['id'],request_sha256=digest(u['request'].encode()),amount=u['amount']) for u in unknown])
