...
const base=process.env.BASE_URL||'http://127.0.0.1:33009',out=path.resolve(process.env.EVIDENCE||'evidence/paid-pilot/browser');fs.mkdirSync(out,{recursive:true});
try{
...
await page.goto(base);await page.locator('#seed').fill('20260921');await page.locator('#generate').click();await page.waitForFunction(()=>window.workbench?.revision);
const initial=await page.evaluate(()=>structuredClone(window.workbench));const reg=JSON.parse(fs.readFileSync('evidence/paid-pilot/registered-candidate.json','utf8'));assert.equal(initial.revision,reg.layout_revision);
...
assert.match(await page.locator('#candidateStatus').innerText(),/secondary_pass_parent_pending/,'saved grounded review must be visible independently of technical candidate metadata');
...
const water=[];initial.layout.materials.forEach((row,r)=>row.forEach((m,c)=>{if(m==='water')water.push([c,r]);}));
...
await page.locator('#finalDensity').selectOption('1');const dl=page.waitForEvent('download');await page.locator('#candidateDownload').click();const download=await dl;await download.saveAs(path.join(out,download.suggestedFilename()));assert.equal(await download.failure(),null);
