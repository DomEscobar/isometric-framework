...
OUT=ROOT/'evidence/code-layout-flat-live-v25';OUT.mkdir(parents=True,exist_ok=True)
if (OUT/'new-run.json').exists():raise SystemExit('already started')
parent_id='31b54f710feb480aaf64ba637d65bb81'
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=120) as c:
    ...
    resp=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'eur50-space-bunny-terraces-v25-20260923'})
    ...
    scope=dict(run_id=run['id'],config_sha256=run['config_sha256'],approval_text='Same explicit EUR50 cap and quality recovery authorization; v25 with palette plurality alignment, reserved brick wall block and exact-size provider.',project_cap_microusd=57_315_000,budget_microusd=budget,max_calls=36,max_images=15,expires=time.time()+7200,liabilities=[dict(call_id=u['id'],request_sha256=digest(u['request'].encode()),amount=u['amount']) for u in unknown])
