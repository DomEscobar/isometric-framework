...
out=ROOT/'evidence/hybrid-board-variants';out.mkdir(exist_ok=True)
...
s=default_store();pid='bf0775b583fc44d689907e042b29f96c'
p,_,cfg=validated_directive(s,parent_id=pid)
assert p['inherited_calls']==7 and p['inherited_images']==1
...
assert account['effective_microusd']==13885141 and account['settled_calls']==9 and len(unknown)==2
assert not s.g.policy['approved'] and s.g.policy['total_usd']=='20.00'
private=ROOT/'data/hybrid-board-variants-before.json'
...
directive=('Owner explicit choice 2026-09-22 (board variants first): richer, more varied FLAT face-on material swatches for ALL seven slots — lively grass tufts and flowers, warm fine-grained sand paths, small varied cobbles for square, subtle flat cut stone for stairs (absolutely NO treads/risers/objects), crisp ashlar wall masonry, shimmering flat water with small lily pads, fine plateau turf. Finer joints and feature scale than the previous board; more natural variation within each swatch; keep every material FLAT and orthographic; preserve each semantic slot exactly; the renderer builds all geometry.')
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=180) as c:
    response=c.post('/api/hybrid-runs/'+pid+'/continue-directive',json=dict(budget_microusd=budget,max_seconds=7200,confirm_paid=True,approval_text=approval,owner_style_directive=directive),headers={'Idempotency-Key':'board-variants-calls4-images1-once-v1'})
    response.raise_for_status();r=response.json();immutable(out/'new-run.json',canonical(r))
    assert (r['call_count'],r['image_count'])==(7,1) and r['config']['max_images']==2 and r['config']['max_calls']==11
    ...
    scope=dict(run_id=r['id'],config_sha256=r['config_sha256'],approval_text=approval,owner_style_directive=directive,
        ...
        max_calls=11,max_images=2,new_images=1,followup_calls=4,settled_calls=9,expires=time.time()+7200,
