"""Free-only billing lookup for the repair child calls. No POST, no liability release."""
...
s=default_store();rid='3c92b111114d479485d3aafe4991d441'
out=ROOT/'evidence/hybrid-reassessment-repair'
...
with s.g.connect() as db:
    for call in db.execute('SELECT id,role,request,receipt,amount FROM hybrid_calls WHERE run=? ORDER BY rowid',(rid,)):
        ...
        if call['role'].startswith('image'):
            entry=dict(call_id=call['id'],role=call['role'],generation_id=None,held_microusd=call['amount'],quoted_usd=str(receipt.get('data',{}).get('price','provider-quote')),note='WaveSpeed image edit; provider billing not independently returned')
            result.append(entry);print(json.dumps(entry));continue
        ...
        entry=dict(call_id=call['id'],role=call['role'],generation_id=gid,held_microusd=call['amount'],
            ...
            model=data.get('model'),finish_reason=data.get('finish_reason'),tokens_prompt=data.get('tokens_prompt'),
