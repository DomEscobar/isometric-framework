# Pixel Borough production contract

Authorized by the user's brief; recorded before asset production and host implementation.

Build a complete compact, playable original creature town using the public framework API. The unchanged `test-results/real-visual-session/user-target.jpg` is the style and composition target, not a source of sprites. Match crisp dark outlines, clustered pixels, common pixel density, readable human/building proportions, varied warm and blue roofs, a compact paved town surrounded by forest and tall grass, natural margins, blue stream, coherent banks and a traversable bridge. No franchise marks or inherited example content.
...
Completion evidence: actual live desktop/mobile captures, ground-only and boundary views, packed preview, full motion cycles, bridge crossing both ways and blocked water/side attempts, creature greeting, input release/pause, renderer and frame timing with blank-page baseline. Run framework freeze/inspect/snapshot/compare/accept workflow. Keep visual, motion, gameplay and performance verdicts separate. Any independent reviewer must inspect actual images. All retained evidence goes under test-results/real-visual-session; no invented successful rounds.

Ownership: examples/pixel-borough and assigned evidence only. No src, skills, other examples, root docs or existing test changes; no commit/push. Existing server http://127.0.0.1:4187 remains owned by its original session. Record capability gaps and use valid host solutions where possible.
