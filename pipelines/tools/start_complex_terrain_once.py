"""ONE authorized live complex-terrain run. Never run as a regression test."""
...
out=ROOT/'evidence/hybrid-complex-terrain';out.mkdir(exist_ok=True)
...
assert account['effective_microusd']==15493816 and account['released_microusd']==0 and len(unknown)==2
assert not s.g.policy['approved'] and s.g.policy['total_usd']=='20.00'
private=ROOT/'data/hybrid-complex-terrain-before.json'
immutable(private,canonical(dict(tables=before,policy=s.g.policy)));private.chmod(0o600)
# Same retained reference + actor as the proven runs.
...
body=dict(mode='live',description=description,constraints=dict(width=24,height=20,actor_width=.64),
    ...
    token_policy=dict(extraction=dict(max_tokens=32768,reasoning_effort='medium'),review=dict(max_tokens=32768,reasoning_effort='medium')),
    **reference)
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=180) as c:
    response=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'complex-terrain-project20-calls6-images1-once-v1'})
    ...
    assert r['config']['auto_continue'] is True and r['config']['token_policy']['extraction']['max_tokens']==32768
    scope=dict(run_id=r['id'],config_sha256=r['config_sha256'],approval_text=approval,amendment=None,
        ...
        description_sha256=digest(description.encode()),expires=time.time()+7200,
    ...
    print(json.dumps(dict(run_id=r['id'],phase=got.json()['phase'],acknowledgment_id=aid,budget_microusd=budget,held_before=account['effective_microusd'])))
