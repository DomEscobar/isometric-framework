"""Create one fresh, EUR50-capped terrain run: exact-size seedream edit provider."""
...
OUT=ROOT/'evidence/code-layout-flat-live-v16';OUT.mkdir(parents=True,exist_ok=True)
if (OUT/'new-run.json').exists():raise SystemExit('already started')
parent_id='1fef8282906746759e058d3ae67a451a'
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=120) as c:
    ...
    resp=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'eur50-flat-terrain-recovery-v16-20260923'})
