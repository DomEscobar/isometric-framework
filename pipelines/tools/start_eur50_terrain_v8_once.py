"""Create one fresh, EUR50-capped, contract-bound terrain recovery run."""
...
OUT=ROOT/'evidence/code-layout-flat-live-v8';OUT.mkdir(parents=True,exist_ok=True)
...
cap=json.loads((ROOT/'evidence/code-layout-flat-live-v7/authorization.json').read_bytes());store=default_store()
...
with store.g.connect() as db:
 ...
 assert before['settled_calls']==22
...
assert 0<headroom<=cap['budget_microusd']
# Four known paid calls have already been used by the abandoned v7 run.
run_max_calls=cap['max_calls']-4
assert run_max_calls>=2*5
...
assert planner_reserve+reviewer_reserve+image_reserve<=headroom
...
with httpx.Client(base_url='http://127.0.0.1:48765',timeout=180) as c:
 ...
 resp=c.post('/api/hybrid-runs',json=body,headers={'Idempotency-Key':'eur50-flat-terrain-recovery-v8-20260923'})
 if resp.status_code!=201:raise RuntimeError(f'run create failed {resp.status_code}: {resp.text}')
 run=resp.json();assert run['config']['layout_contract']=='flat-pond-path-plateau/1'
 assert run['config']['runtime_version']==runtime_version()
 immutable(OUT/'new-run.json',canonical(run))
 scope=dict(run_id=run['id'],config_sha256=run['config_sha256'],approval_text=cap['authorization_text_source']+'; '+cap['notes'],project_cap_microusd=cap['project_cap_microusd'],budget_microusd=headroom,max_calls=run_max_calls,max_images=15,expires=time.time()+7200,liabilities=[dict(call_id=x['id'],request_sha256=digest(x['request'].encode()),amount=x['amount']) for x in unknown])
 ...
 print(json.dumps(dict(run_id=run['id'],phase=read.json()['phase'],planner_model=pm['id'],reviewer_model=rm['id'],project_cap_microusd=cap['project_cap_microusd'],prior_effective_microusd=before['effective_microusd'],run_budget_microusd=headroom,planner_reserve_microusd=planner_reserve,reviewer_reserve_microusd=reviewer_reserve,image_quote_microusd=image_reserve,unknown_holds_retained=len(unknown),authorization_id=aid,runtime_version=runtime_version(),max_calls=run_max_calls,layout_contract=run['config']['layout_contract'])))
